    ...
    for line in lines :
        if (line[0:4] == "ATOM") or ((line[0:6] == "HETATM") and ( (line[17:20].strip() == "MET") or  (line[17:20].strip() == "MSE") )) :
            ...
            if occupancy == alternateoccupancy  or occupancy == " " : # means this atom corresponds to the first rotamer found in the PDB for this residue
                ...
                #if CG :
                    #dPDB[curres]["atomlistTowrite"].append(atomtype.split("_")[1]) # necessary for the writing later
                ...
                if bfactor == True :
                    dPDB[curres][atomtype]["bfactor"] = float(line[60:67].strip())
                #if chargeFromInfile == True :
                #    dPDB[curres][atomtype]["charge"] = float(line[60:67])
                #else :
                #    dPDB[curres][atomtype]["charge"] = charge


            dPDB[code_res]["resnum"] = current_res
            prev_res = cures
            #dPDB[curres]["inser"] =  "%s"%(line[26:27])
...
def writePDB(dPDB, filout = "out.pdb", bfactor = False) :
    ...
    for res in dPDB["reslist"] :
        for atom in dPDB[res]["atomlist"] :
            if bfactor :
                fout.write("ATOM  %5s  %-4s%3s %s%4s    %8.3f%8.3f%8.3f  1.00%7.3f X X\n"%(dPDB[res][atom]["id"], atom, dPDB[res]["resname"],chain, res,dPDB[res][atom]["x"], dPDB[res][atom]["y"],dPDB[res][atom]["z"],dPDB[res]["bfactor"] ))
            else:
                fout.write("ATOM  %5s  %-4s%3s %s%4s    %8.3f%8.3f%8.3f  1.00  1.00 X X\n"%(dPDB[res][atom]["id"], atom, dPDB[res]["resname"],chain, res,dPDB[res][atom]["x"], dPDB[res][atom]["y"],dPDB[res][atom]["z"] ))
